  ...
  private $tbl_roles= 'documents';
   ...
   function get_list() {
	...
	function search($documentcategory_id,$user_id,$year_published,$project_id)
	 {
		$sql = 'SELECT t1.*,t1.id AS document_id,t2.*';
		
		if($user_id !=0)
		{
			$sql .= ',t3.*';
		}
		$sql .= ' FROM documents AS t1, documentcategories AS t2';
		
		if($user_id !=0)
		{
			$sql .= ',users AS t3';
		}
		$sql .= ' WHERE t1.documentcategory_id='.$documentcategory_id.'
				AND t1.documentcategory_id = t2.id';
		
		
		if($user_id !=0)
		{
			$sql .=' AND t1.user_id='.$user_id.' AND t1.user_id = t3.id';
		}
		
		if($year_published !=0)
		{
			$sql .=' AND t1.year_published='.$year_published;
		}
		
		if($project_id !=0)
		{
			$sql .=' AND t1.project_id='.$project_id;
		}
