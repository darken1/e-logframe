...
<script>
   ...
   function trim(str){
	...
	function Getoutputs(frm){
	if(validateForm(frm)){
	document.getElementById('outputs').innerHTML='';
	var url = "<?php echo base_url(); ?>index.php/indicatorstracking/getoutputs";
	...
	if(connection.readyState == 4){
		document.getElementById('outputs').innerHTML=connection.responseText;
		
		
	}
	if((connection.readyState == 2)||(connection.readyState == 3)){document.getElementById('outputs').innerHTML = '<span style="color:green;">Sending request....</span>';}}}
	}
	
	
	function Getprojectindicators(frm){
	if(validateForm(frm)){
	document.getElementById('indicators').innerHTML='';
	var url = "<?php echo base_url(); ?>index.php/indicatorstracking/getoutputindicators";
	
	var params = "projectoutput_id=" + totalEncode(document.frm.projectoutput_id.value );
	var connection=connect(url,params);
	...
	var url = "<?php echo base_url(); ?>index.php/indicatorstracking/gettargets";
	
	var params = "projectoutputindicator_id=" + totalEncode(document.frm.projectoutputindicator_id.value );
	var connection=connect(url,params);
	...
	var url = "<?php echo base_url(); ?>index.php/indicatorstracking/addreach";
	
	var params =  "projectoutputindicator_id="+totalEncode(document.frm.projectoutputindicator_id.value )+ "&reached="+totalEncode(document.frm.reached.value )+ "&report_month="+totalEncode(document.frm.report_month.value )+ "&report_year="+totalEncode(document.frm.report_year.value )+ "&project_id="+totalEncode(document.frm.project_id.value )+ "&comments="+totalEncode(document.frm.comments.value );
	var connection=connect(url,params);
	...
	function viewHistory(frm){
	if(validateForm(frm)){
	document.getElementById('historylist').innerHTML='';
	var url = "<?php echo base_url(); ?>index.php/indicatorstracking/viewhistory";
...
<script>
		...
		function saveToDatabase(editableObj,column,id) {
			...
			$.ajax({
				url: "<?php echo base_url(); ?>index.php/indicatorstracking/editreached",
				type: "POST",
		...
		function saveToTheDatabase(editableObj,column,id) {
			$(editableObj).css("background","#FFF url(<?php echo base_url(); ?>/img/loaderIcon.gif) no-repeat right");
			$.ajax({
				url: "<?php echo base_url(); ?>index.php/indicatorstracking/editcomment",
				type: "POST",
				data:'column='+column+'&editval='+editableObj.innerHTML+'&id='+id,
				success: function(data){
					$(editableObj).css("background","#FDFDFD");
		...
		<body>
			<?php include(APPPATH . 'views/common/navigation.php'); ?>
				...
				<div class="breadcrumbs">
					<ul>
						...
						<li>
							<a href="<?php echo base_url() ?>index.php/indicatorstracking/add">indicators tracking</a>
						</li>
					</ul>
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					  <select name="project_id" id="project_id" onChange="Getoutputs(this)" required="required" class='chosen-select form-control'>
            ...
            <div class="form-group">
				...
				<div class="col-sm-10">
					 <div id="outputs">
                                            <select name="projectoutput_id" id="projectoutput_id" class='chosen-select form-control' required="required">
